...
logs by setting an environment variable. The library works for server1-side and
client-size applications so it's great for writing isomorphic JavaScript.

The debug output can be triggered using environment variables on the server1 and
...
2. `options` These options can only be applied to the server1, not client code:
...
that no longer can use environment variables as these only work on the server1.
...
Unlike the server1, the output of the browser is not colored. The reason for this
...
> Please note that this feature is server1-side only as in the browser we can only
> output to the console

The beauty of this logger is that it allows a custom stream where you can write
the data to. So you can just log it all to a separate server1, database and what
